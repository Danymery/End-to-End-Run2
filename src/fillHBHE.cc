...
TH2F *hEvt_HBHE_energia;
TProfile2D *hHBHE_energia_EB;
TProfile2D *hHBHE_energia;
std::vector<float> vHBHE_energia_EB_;
std::vector<float> vHBHE_energia_;

void RecHit2::branchesHBHE ( TTree* tree, edm::Service<TFileService> &fs ) {

  // Branches for images
 tree->Branch("HBHE_energia_EB", &vHBHE_energia_EB_);
 tree->Branch("HBHE_energia",    &vHBHE_energia_);
  
// Intermediate helper histogram (single event only)
  hEvt_HBHE_energia = new TH2F("evt_HBHE_energia", "E(i#phi,i#eta);i#phi;i#eta",
      HBHE_IPHI_NUM,           HBHE_IPHI_MIN-1,    HBHE_IPHI_MAX,
      2*(HBHE_IETA_MAX_HE-1),-(HBHE_IETA_MAX_HE-1),HBHE_IETA_MAX_HE-1 );

  // Histograms for monitoring
  hHBHE_energia = fs->make<TProfile2D>("HBHE_energia", "E(i#phi,i#eta);i#phi;i#eta",
      HBHE_IPHI_NUM,      HBHE_IPHI_MIN-1, HBHE_IPHI_MAX,
      2*HBHE_IETA_MAX_HE,-HBHE_IETA_MAX_HE,HBHE_IETA_MAX_HE );
      hHBHE_energia_EB = fs->make<TProfile2D>("HBHE_energy_EB", "E(i#phi,i#eta);i#phi;i#eta",
      HBHE_IPHI_NUM, HBHE_IPHI_MIN-1,HBHE_IPHI_MAX,
      2*HBHE_IETA_MAX_EB,               -HBHE_IETA_MAX_EB,               HBHE_IETA_MAX_EB );

}

void RecHit2 :: fillHBHE( const edm::Event& iEvent, const edm::EventSetup& iSetup ){
  
  int iphi_, ieta_, ietaAbs_, idx_;
  float energy_;

  vHBHE_energia_EB_.assign( 2*HBHE_IPHI_NUM*HBHE_IETA_MAX_EB, 0. );
  vHBHE_energia_.assign( 2*HBHE_IPHI_NUM*(HBHE_IETA_MAX_HE-1), 0. );
  hEvt_HBHE_energia->Reset();

  edm::Handle<HBHERecHitCollection> HBHERecHitsH_;
  iEvent.getByToken( HBHERecHitT_, HBHERecHitsH_ );
...
for ( HBHERecHitCollection::const_iterator iRHit = HBHERecHitsH_->begin();
	...
	// Fill vectors/histos by ieta range: /////////////////////////
	   ...
	   // Fill histograms for monitoring
      	  hHBHE_energia->Fill( iphi_  , ieta_, energy_*0.5 );
      	  hHBHE_energia->Fill( iphi_+1, ieta_, energy_*0.5 );
      
	  // Fill intermediate helper histogram
      	  hEvt_HBHE_energia->Fill( iphi_  , ieta_, energy_*0.5 );
      	  hEvt_HBHE_energia->Fill( iphi_+1, ieta_, energy_*0.5 );
          continue;}

	// (B) hId.ieta() <= 20: fine iphi granularity (beyond this, iphi granularity is halved)
    	// Fill histograms normally
    	else {
      		hHBHE_energia->Fill( iphi_,ieta_,energy_ );
      		hEvt_HBHE_energia->Fill( iphi_,ieta_,energy_);}
	...
	// Fill histograms for monitoring
    	hHBHE_energia_EB->Fill( iphi_,ieta_,energy_ );
	...
	// Fill vector for image
	 ...
	 vHBHE_energia_EB_[idx_] += energy_;
 ...
 // Fill vector for full HBHE image using helper histogram
  for (int ieta = 1; ieta < hEvt_HBHE_energia->GetNbinsY()+1; ieta++) {
    for (int iphi = 1; iphi < hEvt_HBHE_energia->GetNbinsX()+1; iphi++) {

      energy_ = hEvt_HBHE_energia->GetBinContent( iphi, ieta );
      if ( energy_ <= 0 ) continue;
      idx_ = (ieta-1)*HBHE_IPHI_NUM + (iphi-1);
      // Fill vector for image
      vHBHE_energia_[idx_] = energy_;
